...
class KalmanObject:
    ...
    def kalman_update(self, uu, zz):
        ...
        self.K = self.P.dot(self.H.T).dot(np.linalg.inv(
...
def get_largest_face(dets):
    ...
    face_areas = [(det.right()-det.left())*(det.bottom()-det.top())
                  for det in dets]
...
def get_pose_estimation(img_size, image_points):
    ...
    (success, rotation_vector, translation_vector) = cv2.solvePnP(
        model_points, imagePoints, camera_matrix, dist_coeffs, flags=cv2.SOLVEPNP_DLS)
...
if __name__ == '__main__':
    ...
    # Tune Q, R to change landmarks_x sensitivity
    KalmanX = KalmanObject(POINTS_NUM_LANDMARK, 1, 10)
    # Tune Q, R to change landmarks_y sensitivity
    KalmanY = KalmanObject(POINTS_NUM_LANDMARK, 1, 10)
    ...
    while (True):
        ...
        landmarks = mean_filter_for_landmarks(landmarks)
        leftEyeWid, rightEyewid, mouthWid, mouthLen = get_feature_parameters(
            landmarks_orig)
        parameters_str = 'leftEyeWid:{}, rightEyewid:{}, mouthWid:{}, mouthLen:{}'.format(
            leftEyeWid, rightEyewid, mouthWid, mouthLen)
...
#        image_points = np.vstack((landmarks[30],landmarks[8],landmarks[36],landmarks[45],landmarks[48],landmarks[54]))
        ...
        data = str(translation_vector[0, 0])+':'+str(translation_vector[1, 0])+':'+str(translation_vector[2, 0])+':'+str(
            w)+':'+str(x)+':'+str(y)+':'+str(z)+':'+str(leftEyeWid)+':'+str(rightEyewid)+':'+str(mouthWid)+':'+str(mouthLen)
        labels = {'datas': data}
        with open('../Assets/test2.json', 'w') as fp:
            json.dump(labels, fp)
        # try:
        #     print(data)
        ...
        (nose_end_point2D, jacobian) = cv2.projectPoints(np.array(
            [(0.0, 0.0, 1000)]), rotation_vector, translation_vector, camera_matrix, dist_coeffs)

        for i in range(8):
            (boxPoints2D[:, :, i, :], jacobian) = cv2.projectPoints(np.array(
                [boxPoints3D[i]]), rotation_vector, translation_vector, camera_matrix, dist_coeffs)
        ...
        boxset_3 = np.vstack(
            (boxPoints2D[0, 0, 0, :], boxPoints2D[0, 0, 4, :]))
        boxset_4 = np.vstack(
            (boxPoints2D[0, 0, 1, :], boxPoints2D[0, 0, 5, :]))
        boxset_5 = np.vstack(
            (boxPoints2D[0, 0, 2, :], boxPoints2D[0, 0, 6, :]))
        boxset_6 = np.vstack(
            (boxPoints2D[0, 0, 3, :], boxPoints2D[0, 0, 7, :]))
